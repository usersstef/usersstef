...
src_file = r"D:/Python/Filename_generator/ApexVO"   # Folder with source files
...
template_file = os.path.join(src, f"{prefix}ID_GER_DE.wav")   # Check if the template file exists
if not os.path.exists(template_file):
    print(f"\nError: Template file {template_file} not found.")
    exit(1)

for i in range(1, num + 1):
    dest_file = os.path.join(src, f"{prefix}ID{i}_GER_DE.wav")
