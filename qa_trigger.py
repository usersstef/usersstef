...
""" while True:
    driver.get(HangfireQA); time.sleep(1)
    for i in range(3):
        checkbox()
    driver.refresh(); time.sleep(1)
    print("Trigger actions finished & page refreshed, use Ctr + c to stop the script")
    time.sleep(1) """

driver.get(HangfireQA); time.sleep(1)
for i in range(3):
    trigger()
driver.refresh(); time.sleep(0.5); driver.close()
